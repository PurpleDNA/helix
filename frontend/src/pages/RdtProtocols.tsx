...
function fetchTimeline(params: RunParams, signal: { ws?: WebSocket }) {
  return new Promise<RawEvent[]>((resolve, reject) => {
    ...
    ws.onerror = () =>
      reject(
        new Error("could not reach the simulator — is the backend running?"),
      );
  });
}
...
export default function RdtProtocols() {
  ...
  useEffect(() => {
    ...
    fetchTimeline(submitted, signal)
      .then((events) => {
        ...
        const startPaused = matchMedia(
          "(prefers-reduced-motion: reduce)",
        ).matches;
        const player = new StagePlayer(
          domRef.current as StageDom,
          run,
          startPaused,
        );
  ...
  const maxWindow =
    protocol === "selective_repeat"
      ? Math.max(1, Math.floor(nMessages / 2))
      : 15;
  const parsed: RunParams = {
    ...
    corrupt:
      clamp(parseInt(fields.corrupt, 10), 0, 70, DEFAULTS.corrupt * 100) / 100,
    // Stop-and-wait is window=1 by definition.
    window:
      protocol === "stop_and_wait"
        ? 1
        : clamp(parseInt(fields.window, 10), 1, maxWindow, DEFAULTS.window),
  ...
  const runScenario = (params: RunParams) => {
    ...
    setFields((f) => ({
      ...
      window:
        params.protocol === "stop_and_wait" ? f.window : String(params.window),
      rto: String(params.rto),
    }));
    setSubmitted(params);
    setRunId((i) => i + 1);
  ...
  return (
    <div className="rdt">
      <header className="rdt-head">
        ...
        <p>
          Pick a protocol, set the channel conditions, and watch the sender earn
          every delivery.
        </p>
      </header>
      ...
      <div className="rdt-panel" id="instrument">
        <div className="readouts">
          <span className="readout">
            ...
            <span className="v">
              {PROTOCOLS.find((p) => p.id === active.protocol)?.label}
            </span>
          </span>
          <span className="readout">
            <span className="k">win</span>
            <span className="v">
              {active.protocol === "stop_and_wait" ? 1 : active.window}
            </span>
          </span>
          <span
            className="readout"
            title="retransmission timeout, in simulated seconds"
          >
